...
public class Grid
{
    ...
    public Grid(int width, int height, float cellSize, Vector3 originPosition)
    {
        ...
        for(int x=0;x < gridArray.GetLength(0);x++)
        {
            ...
            {
                debugTextArray[x,y] = UtilsClass.CreateWorldText(gridArray[x,y].ToString(), null, GetWorldPosition(x,y) + new Vector3(cellSize,cellSize)/2, 10,Color.white, TextAnchor.MiddleCenter);
    ...
    public void SetValue(int x, int y, int value)
    {
        ...
        {
            gridArray[x, y] = Mathf.Clamp(value, HEAT_MAP_MIN_VAL, HEAT_MAP_MAX_VAL);
